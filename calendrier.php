						...
						<tbody>
							<tr>
								...
								foreach ($jour as $j=>$w) {
									...
									if($j == 1 && $w != 1){
										$col= $w-1;
										echo "<td colspan='".$col."' class='padding'></td>";
									}
									echo "<td>
											<div class='relative'>
												<div class='day'>".$j."</div>
											...
											<div class='daytitle'>
												".$date->jourSemaine[$w-1]." ".$j." ".$date->Mois[$m-1]."
											</div>";
...
<?php
	...
	function rdvMedecin($idMed, $annee){
			...
			$r = array();
			$resCal = $linkpdo->query("SELECT * FROM rdv WHERE idMed='$idMed' ORDER BY dateRdv DESC, heure DESC"); 
			while($data = $resCal->fetch()) { 
